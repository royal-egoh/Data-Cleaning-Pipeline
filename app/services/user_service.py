...
oauth_scheme = OAuth2PasswordBearer(tokenUrl="/user/login")  # ?Login Endpoint
...
def create_user(user: UserCreate, db: Session):
    ...
    existing_user = db.execute(select(User).where(or_(
        User.email == user.email, User.username == user.username))).scalar_one_or_none()
    if existing_user:
